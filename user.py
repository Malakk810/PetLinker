...
def login_user():
    ...
    return jsonify(result), 200 if result["success"] else 400  # 200 for successful login

@user.route("/user/edit/<string:username>", methods=["PUT"])
def edit_user(username):
    data = request.json
    result = user_service.edit_user_profile(username, data)
    return jsonify(result), 200 if result["success"] else 400  # 400 for unsuccessful update
