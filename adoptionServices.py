...
class AdoptionService:
    ...
    def get_all_pets(self):
        self.cursor.execute("SELECT Licence_number, Gender, Age, Health_Condition,Type_, Status_ FROM pets WHERE Status_='Available',Location")
        pets = self.cursor.fetchall()
        return [{"Licence_number": row[0], "Gender": row[1], "Age": row[2], "Health_condition": row[3],"Type_": row[4], "Status_": row[5]} for row in pets]
